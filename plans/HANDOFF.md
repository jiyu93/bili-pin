...
3. **数据持久化**
   ...
   - 自动数据迁移和去重
   - **仅存储真实 `mid`（数字 uid）**，不再把头像 hash 当成 uid 持久化
...
### 优先级1：修复Bug #1（核心问题）
1. 以 portal 接口为 mid 来源，稳定映射推荐横条每个头像到 mid
2. 确认 `feed/all` host_mid 改写后能覆盖分页/刷新等场景（必要时改写 offset 请求）
3. 完成后移除旧的 `src/bili/feedFilter.ts` 历史方案（保留注释即可）

### 优先级2：修复Bug #2（高亮显示）
1. 添加选中状态的CSS样式
2. 在点击置顶UP时添加选中类
3. 可能需要监听Feed变化来判断当前选中的UP

### 优先级3：实现拖拽排序
1. 使用HTML5 Drag & Drop API或第三方库
2. 添加排序字段到存储
3. 渲染时按排序字段排序
...
3. **幂等性**：所有UI注入必须保证幂等，避免重复渲染
4. **数据迁移**：`src/storage/pins.ts` 已实现自动数据迁移，但新增字段时可能需要考虑兼容性
...
**交接完成**：请新会话先阅读此文档，然后从“下一步行动”的优先级1开始。
